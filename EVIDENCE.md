...
- **実装**: 誤概念ID別の説明、段階ヒント、答え閲覧後の別問題を用意する。端末内AIへの質問は、正解前には完成コードを示さない段階ヒントとして扱い、利用後の正解を「無支援」と判定しない。
- **限界**: 即時・遅延の優劣は課題に依存する。初学者のコード練習では誤りを固定しないよう、その場で具体的に返す。
- **AI利用上の限界**: 生成された説明は誤る可能性があり、正確性や学習効果を保証しない。公式資料への導線と、教材側で検証済みのヒントを残す。
- **根拠**:
...
- **実装**: 1チャンク1中心目標、自己ペースの会話、実行順の強調、最初に操作できる完成見本を示し、同じ注文データをJSのDOM画面、TSの型、NodeのAPIへ引き継ぐ。
- **限界**: 面白いだけで目標と無関係な詳細は転移を妨げる。飾りの画像、重複文章、強制アニメーションは加えない。注文管理の完成見本は概念同士の関係を示す足場であり、物語にするだけで記憶が高まるとは扱わない。
- **根拠**:
