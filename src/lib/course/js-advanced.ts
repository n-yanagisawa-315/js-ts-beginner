...
p.then((value) => {
  console.log(value);
      ...
      },
      {
        id: "q2",
        slide: 1,
        prompt: "注文数の初期値 1 に追加注文の 1 件を足し、その合計を2個セット換算で2倍して、最終個数だけを表示してください。",
...
load();`,
      ...
      },
      {
        id: "q3",
        slide: 2,
        prompt: "プロフィール `pa` と通知設定 `pb` を同時に取得する想定で、2つの Promise をまとめて待ち、結果を順番に表示してください。",
        ...
        fileName: "script.js",
        steps: [
          "2つの Promise をまとめて待ち、両方の結果を受け取る",
    ...
    questions: [
      {
        id: "q1",
        slide: 0,
        prompt: "画面の開始ログ `A` と受付完了ログ `C` はすぐ出し、通知ログ `B` だけを0ミリ秒タイマーで後から表示してください。",
...
import main from "./app.js";`,
    ...
    questions: [
      ...
      },
      {
        id: "q2",
        slide: 1,
        prompt: "計算モジュール `m.js` が名前付きで公開した `add` を、注文金額の計算ファイルで受け取る方法を選んでください。",
        lead: "入力側では公開名 `add` をそのまま利用する必要があります。主な値を任意名で受け取る default import ではなく、名前付き公開に対応する読み込み方を見分けてください。",
        kind: "choice",
        options: [
          'import default add from "./m.js"',
          'import { add } from "./m.js"',
          'const { add } = require("./m.js")',
          'import { add } from "../m.js"',
        ],
        steps: [
