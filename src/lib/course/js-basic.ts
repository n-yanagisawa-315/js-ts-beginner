...
console.log(cars[cars.length - 1]); // "最後"`,
      },
      {
        ...
        points: [
          "push: 末尾に足して、新しい length を返す",
          "pop: 末尾を取り除いて、その値を返す",
          "slice(1, 3) は 1 以上 3 未満。元は変わらない",
        ],
        talk: [
          {
            "speaker": "beginner",
            "text": "pushとsliceは、どちらも新しい配列を返して元は変えないですか？"
          },
          {
            "speaker": "engineer",
            "text": "違います。pushは元へ追加し、sliceは範囲をコピーします。"
          },
          {
            "speaker": "beginner",
            "text": "constの配列をpushで変えられるのは、再代入ではないから？"
          },
          {
            "speaker": "engineer",
            "text": "そうです。pushは新しいlengthを返し、popは末尾を外してその値を返します。sliceの終了位置は含みません。似た名前のspliceは本体を切るので混同に注意してください。"
          }
        ],
        diagram: "array",
        code: `const xs = [1, 2];
...
if (score >= 80) {
  ...
  console.log(i);
      ...
      },
      {
        id: "q6",
        slide: 5,
        prompt: "3件のデータを順に確認する番号として、starterのiを0から2まで表示してください。",
        lead: "iは0で用意されています。3未満の間だけ繰り返し、現在の番号を表示してから1増やしてください。0、1、2が表示された後に必ず終了すれば完成です。",
        kind: "code",
        starter: "let i = 0;\n// while で 0, 1, 2 と出す\n",
        fileName: "script.js",
        steps: [
        ...
        hint: "ループの中で、条件に使う値を必ず変化させます。今回は表示と更新の順番が出力内容を決めます。",
        sample: "0\n1\n2",
        answer: `let i = 0;
while (i < 3) {
  console.log(i);
  i++;
      ...
      },
      {
        ...
        hint: "表示より前に、飛ばす条件と終了する条件を確認します。それぞれの制御が「現在の回だけ」か「ループ全体」かを区別してください。",
        sample: "0\n1\n3",
