...
function pickToken(line: string, label?: string): string | undefined {
  ...
  if (label) {
    const semantic: Array<{ when: RegExp; token: string | RegExp }> = [
      { when: /addでクラス|classList\.add/, token: "add" },
      { when: /removeでクラス|classList\.remove/, token: "remove" },
      { when: /まとめて受け|複数選|querySelectorAll/, token: "querySelectorAll" },
      { when: /タグ名から|createElement/, token: "createElement" },
      { when: /末尾へ追加|\bappend\b/, token: "append" },
      { when: /eventは発生/, token: "event" },
      { when: /stringifyして保存|して保存/, token: "saveOrders" },
      { when: /HTML文字列へ連結しない/, token: "createElement" },
      { when: /#|idを選/, token: "#" },
      ...
      { when: /木の入口/, token: "document" },
      { when: /\bdocument\b/, token: "document" },
    ];
...
function bestLineForLabel(lines: string[], label: string): number {
  const cues: Array<{ label: RegExp; code: RegExp; preferLast?: boolean }> = [
    ...
    { label: /addでクラス|classList\.add/, code: /classList\.add\b/ },
    { label: /removeでクラス|classList\.remove/, code: /classList\.remove\b/ },
    { label: /classList|クラスを付/, code: /classList/ },
    ...
    { label: /末尾へ追加|子要素を親|\bappend\b/, code: /\.append\b/ },
    { label: /HTML文字列へ連結しない|createElementで作る|タグ名から/, code: /createElement/ },
    { label: /stringifyして保存/, code: /saveOrders|stringify|setItem/ },
