...
console.log(wrong); // 5。もう実行済みの缶`,
      ...
      {
        title: "渡すと「あとで呼んでもらう」",
        lead: "コールバックは、自分では () を付けず、相手に渡す関数です。郵便の「不在票を見たらかけて」と同じで、かけるタイミングは相手が決めます。次の講義で詳しく扱う forEach に渡す関数が、その典型です。",
        points: [
          "渡す側は、中身を今は実行しない",
          "受け取る側が、準備できたら呼ぶ",
        ...
        talk: [
          ...
          {
            "speaker": "engineer",
            "text": "時間の後とは限りません。呼ぶタイミング・回数・引数を受け取った側に任せる関数です。今すぐ呼ぶ相手も、あとで呼ぶ相手も、渡し方は同じです。"
          },
          {
            "speaker": "beginner",
          ...
          {
            "speaker": "engineer",
            "text": "いいえ。forEachが各要素の準備をしてshowを呼びます。渡す側は()を付けず今は実行せず、受け手がいつ・何回・どんな引数で呼ぶかを決めます。"
          }
        ],
        diagram: "callback-flow",
...
// 1回だけ`,
      },
      {
        ...
        lead: "配列メソッドが (x) => ... を呼ぶとき、今の要素を渡しています。受け手が fn(値) と書くと、渡した関数の仮引数にその値が入ります。作業場に品物を乗せるイメージです。",
        points: [
          "fn(x) は、コールバックへ材料を渡す",
