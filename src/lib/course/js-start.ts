...
// 止まらなければ、下の行まで進む`,
    ...
    questions: [
      ...
      },
      {
        id: "q2",
        slide: 1,
        prompt: "「1行です」という文字列を、確認場所へ1行だけ表示してください。",
        lead: "命令名の大文字・小文字や記号が少し違うだけで、機械は別の命令として扱います。JavaScriptが読める1行を、自分で書いてください。",
        kind: "code",
        starter: "// 1行だけ表示する\n",
        fileName: "script.js",
        steps: [
...
console.log(true);`,
      ...
      },
      {
        ...
        slide: 1,
        prompt:
          '数値の `3` と、文字列の `"3"` について、値の種類をこの順で表示してください。',
        lead:
          "見た目が同じ3でも、数値と文字列では種類が異なります。値の種類は `typeof` で調べ、結果は `console.log` で2行に分けて表示します。1行目は数値の種類名、2行目は文字列の種類名になります。",
        kind: "code",
        ...
        hint:
          "値の種類を調べる `typeof` は、調べたい値の手前で使います。引用符の有無を変えて比較しましょう。",
        sample: "number\nstring",
...
console.log(total);`,
      ...
      },
      {
        ...
        slide: 5,
        prompt:
          "constのuserが指すオブジェクトのname欄だけを「Ren」へ更新し、そのnameを表示してください。",
        lead: "userは同じオブジェクトを指し続けます。名前を別のオブジェクトへ付け替えず、同じオブジェクトの欄だけを更新してください。",
        kind: "code",
        starter: 'const user = { name: "Aya" };\n// name欄だけを更新して表示\n',
        fileName: "script.js",
        steps: [
          "userという名前の指す先は変えない",
          "同じオブジェクトのname欄だけを更新する",
          "更新後のnameを表示する",
        ],
...
console.log(3 >= 3); // true。同じも含む`,
      ...
      {
        title: "この講義の要点",
        lead: "number の + は電卓。string が混ざると文字の連結。埋め込みはテンプレート。入力は Number。比較は ===。次は動的型です。",
        points: [
          "演算の意味は値の種類が決める",
          "入力値は string になりやすい",
        ],
...
console.log(10 % 3);`,
      ...
      },
      {
        id: "q2",
        slide: 1,
        prompt: "数値1と入力欄から届いた文字列「2」をつないだ結果と、コード片「A」と「B」をつないだ結果を順に表示してください。",
        lead: "入力値に文字列が混ざると、足し算のつもりでも文字の連結になります。1行目に12、2行目にABを表示し、値の種類で結果が変わることを確認してください。",
        kind: "code",
        starter: "// + が文字の連結になる例\n",
        fileName: "script.js",
        steps: [
          "数値1と文字列「2」、続けて文字列「A」と文字列「B」を連結して表示する",
          "文字列「A」と文字列「B」を連結して表示する",
...
console.log("A" + "B");`,
        explain:
          '片方が文字だと + は文字をつなぎます。1 + "2" は 3 ではなく "12" です。',
      },
      {
        id: "q3",
        slide: 2,
...
console.log(Number(rawOrderCount) + 1);`,
      ...
      },
      {
        ...
        fileName: "script.js",
        steps: [
          "数値1と文字列「1」を、種類まで見て比較し表示する",
          "同じIDを、変換を許して比較し表示する",
          "先に種類まで見る比較、次に変換を許す比較の順か確認する",
        ],
...
console.log(typeof x); // "string"`,
      ...
      {
        title: '1 + "2" は静かに事故る',
        lead: 'エラーにならずに変な結果が出るのが、動的型の落とし穴です。個数の 1 と札の "2" を足すと、エンジンは黙って文字をつなぎ "12" にします。止まらないので、気づくのが遅れます。種類を揃えてから計算します。',
        points: [
          "1 + 2 は 3。電卓",
          '1 + "2" は "12"。エラーではない',
...
console.log(typeof null); // "object"。空なのに object`,
      ...
      {
        title: "真になる値・偽になる値",
        lead: 'if (value) は、値が true そのものかを見るのではありません。スイッチがオンに見えるかを見ます。falsyはオフ扱いされる値の総称で、false（スイッチオフ）、0（個数ゼロ）、""（空の札）、null（意図した空）、undefined（未設定）、NaN（壊れた数）の6つです。truthyはそれ以外のよく使う値です。空の配列 [] や文字列 "0" も、入れ物や文字があるので truthy です。',
        points: [
          "falsy は6つだけ。true / false という値そのものとは別の、条件での扱いの名前",
          '数値 0 はオフ、文字列 "0" はオン。空文字 "" はオフ、空白 " " はオン',
          "[] と {} は中身が空でも、入れ物があるのでオン",
        ],
        talk: [
          {
            speaker: "beginner",
          ...
          {
            speaker: "engineer",
            text: "なりません。ifはtrueそのものかを見るのではなく、スイッチがオフに見える決まった値かを見ます。オフ扱いをfalsy、オン扱いをtruthyと呼びます。空の配列[]や空のオブジェクト{}は、入れ物として存在するのでtruthyです。中が空でも、箱自体はある、と考えてください。",
          },
          {
            speaker: "beginner",
          ...
          {
            speaker: "engineer",
            text: "違います。数値0は個数がゼロなのでfalsy。文字列「0」はゼロという文字が1個ある札なのでtruthy。空文字は文字が1個もなくfalsy。空白1個は空白という文字があるのでtruthyです。見た目が似ても、種類と中身の有無で分かれます。",
          },
          {
            speaker: "beginner",
            text: "falsyに入るnull、undefined、false、NaNは、それぞれ何がオフなのですか？",
          },
          {
            speaker: "engineer",
            text: "falseはスイッチそのもののオフです。nullは意図して空にした印、undefinedはまだ値を入れてない状態、NaNは数の計算が壊れた印です。falsyはこの4つに、0と空文字を足した6つだけ。ほかのよく使う値はほぼtruthyです。",
          },
        ],
        diagram: "truthy",
        code: `if (0) { /* 入らない */ }
...
if ([]) { /* 入る */ }`,
        ...
        codeExample: `console.log(Boolean(0)); // false。個数ゼロ
console.log(Boolean("0")); // true。札に文字がある
console.log(Boolean("")); // false。空の札
console.log(Boolean([])); // true。列はある`,
        watch:
          "空配列を if に入れると中へ進みます。空かどうかは length で見ます。",
      },
      {
        title: "NaN は「数ではない数」",
...
console.log(Number.isNaN(Number("abc"))); // true`,
      },
      {
        ...
        talk: [
          ...
          {
            "speaker": "engineer",
            "text": "typeofには例外があるので、配列やNaN、nullには専用の確認方法を使います。条件ではfalsyが6つだけ（false、0、空文字、null、undefined、NaN）で、空配列はオン扱いです。次はオブジェクトと配列の扱いへ進みましょう。",
          },
        ],
        diagram: "dynamic",
      },
...
console.log(orderCount + Number(rawOrderCount));`,
      ...
      },
      {
        id: "q3",
        slide: 2,
        prompt: "空の配列 `[]` について、一般的な型名と、配列かどうかの判定結果をこの順で表示してください。",
        lead: "調べる対象は空の配列 `[]` です。先にざっくりした型名、次に配列かどうかを専用の判定で調べ、この順で表示します。配列は一般的な型名ではオブジェクト扱いになる、というクセを確認します。",
        kind: "code",
        starter: "// typeof のクセを確認\n",
        fileName: "script.js",
        steps: [
          "空の配列 `[]` の、ざっくりした型名を調べて表示する",
          "同じ `[]` が配列かどうかを、専用の判定で調べて表示する",
          "型名の次に、配列かどうかの判定結果が出るか確認する",
        ],
        hint: "値の種類を調べる演算子と、配列だけを確かめる機能を、同じ空配列に使います。先に型名、次に判定です。",
        sample: "object\ntrue",
...
console.log(Array.isArray([]));`,
      ...
      },
      {
        id: "q4",
        slide: 3,
        prompt: "在庫数を表す数値 `0` と、入力欄に入った文字列 `\"0\"` を、それぞれ真偽値へ変換してこの順で表示してください。",
        lead: "調べる対象は数値の `0` と、文字列の `\"0\"` です。真偽値へ直すには `Boolean` を使います。個数ゼロはオフ、札のゼロは文字があるのでオン、という違いをこの順で見ます。",
        kind: "code",
        starter: "// falsy と truthy\n",
        fileName: "script.js",
        steps: [
          "数値 `0` を `Boolean` で真偽値へ変換して表示する",
          "文字列 `\"0\"` を `Boolean` で真偽値へ変換して表示する",
          "個数のゼロと、札のゼロで結果が分かれるか確認する",
        ],
        hint: "真偽値へ変換する `Boolean` に、調べたい値を渡します。文字列は引用符で囲みます。",
        sample: "false\ntrue",
