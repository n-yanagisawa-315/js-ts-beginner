...
// 先に書いた行が先。並びを変えると表示順も変わる`,
      ...
      },
      {
        ...
        lead: "console.log は、厨房の中身を窓の外に見せる行為です。英語の log は日誌、console は操作盤です。プログラミングでは「確認用の窓に書け」という意味で使います。見せても、厨房の材料そのものは消えません。計算や代入ではなく、確認です。わからないときは、まずこの窓に出して目で見ます。",
        points: [
          "括弧の中が「何を見せるか」。外の console.log が「見せるという行為」",
          "写真を撮っても被写体は残る、と同じ。値は消えない",
          "ブラウザなら開発者ツールの Console、Node ならターミナルに出る",
        ],
        talk: [
          {
            "speaker": "beginner",
            "text": "console.log(n + 1) と書いたら、nも1増えるんでしょうか？"
          },
          {
            "speaker": "engineer",
            "text": "増えません。計算結果を窓に見せているだけです。"
          },
          {
            "speaker": "beginner",
            "text": "写真を撮るみたいに確認するだけで、元の値はそのままということですか？"
          },
          {
            "speaker": "engineer",
            "text": "そうです。ブラウザならConsole、Nodeならターミナルに出ます。値を変えるには計算結果を変数へ書き戻す必要があり、logは途中経過の確認に使います。"
          }
        ],
        diagram: "sequence",
        code: `let n = 3;
console.log(n); // 3 と表示。n はまだ 3
console.log(n + 1); // 4 と表示。n はまだ 3`,
        codeCaption: "見せても値は変わらない",
        codeExample: `let n = 3;
n + 1;
console.log(n); // まだ 3。書き戻していない`,
        note: "log の中で計算しても、変数へ書き戻さなければ元の値は変わりません。",
      },
      {
        title: "引用符と括弧はセットで使う",
...
console.log("さん");`,
      ...
      },
      {
        id: "q4",
        slide: 3,
        prompt: "在庫数nに商品が1個加わった場合の数を試算し、そのあと現在の在庫数も表示してください。",
        lead: "starterには現在の在庫数としてnが3で用意されています。1行目に1個加えた試算結果、2行目に変更していないnを表示し、4、3の順になれば完成です。",
        kind: "code",
        starter: "let n = 3;\n",
        fileName: "script.js",
        steps: [
        ...
        hint: "表示命令へ式を渡すことはできますが、代入しない限りnの指す値は変わりません。",
        sample: "4\n3",
        answer: `let n = 3;
console.log(n + 1);
console.log(n);`,
        explain: "最初の4は計算結果を見せただけです。書き戻していないのでnは3のままです。",
      },
      {
        id: "q5",
        slide: 4,
        prompt: "案内画面に、あいさつの「こんにちは」と待ち人数の3を、この順で1行ずつ表示してください。",
        lead: "あいさつは文字列、待ち人数は計算にも使える数値として用意します。実行後に「こんにちは」と3が別々の行へ表示されれば完成です。",
        kind: "code",
        starter: "// 言葉は引用符、数はそのまま\n",
        fileName: "script.js",
        steps: [
          "1つ目を文字列として表示する",
          "2つ目を数値として表示する",
...
console.log(x);`,
      ...
      },
      {
        id: "q5",
        slide: 4,
        prompt: "name欄にAyaを持つuserオブジェクトを作り、そのname欄の値だけを表示してください。",
        lead: "関連する値を名前付きの欄にまとめるのがオブジェクトです。userという名前で1つのオブジェクトを作り、name欄には文字列Ayaを持たせます。最後はオブジェクト全体ではなく、name欄だけを表示してください。",
        kind: "code",
        starter: "// 塊も値です\n",
        fileName: "script.js",
        steps: [
        ...
        hint: "オブジェクトには欄の名前と値の組を持たせます。表示時は、userから固定名の欄をたどる方法を使います。",
        sample: "Aya",
...
console.log(user.name);`,
  ...
  },
  {
    ...
    slides: [
      {
        title: "名前は値への付箋",
        lead: '変数（へんすう）は、値をあとから読み書きするために付ける名前です。箱に詰める、より「荷物に付箋を貼る」方が正確です。同じ数字 20 を何度も書き写す代わりに、age と書いてたどれます。let name = "Aya" では、まず右側が表す値を決めます。この「式を計算して1つの値を決めること」を評価と呼びます。そのあと文字列 Aya と左側のnameを結び付けます。あとからnameと書くと、その名前から値を読みます。',
        points: [
          "let は「これから名前を使うよ」という宣言。右が荷物、左が付箋",
          "右辺を先に計算し、1つの値を決める。この処理を評価という",
          "左辺の名前がその値を指す（付箋を貼る）",
        ],
        talk: [
          {
          ...
          {
            "speaker": "beginner",
            "text": "let name = ... なら、左から順に箱を作って中身を入れるんですよね？"
          },
          {
            "speaker": "engineer",
            "text": "実際は右側を先に計算して値を決めます。この処理を評価と呼びます。その後で左の名前を結び付けます。あとでnameと書くとその値を読み、何度読んでも値は消えません。"
          }
        ],
        diagram: "label",
        code: `let name = "Aya";
...
console.log(age); // 21。今の値を読んで1足し、同じ名前へ`,
      ...
      {
        title: "let は付け替え可、const は不可",
        lead: "let はあとから別の値へ名前を付け直せます。const は「この付箋は剥がさない」約束です。値の中身がオブジェクトなら、引き出しの中は変えられる点に注意してください。凍るのは付箋であり、荷物の中身とは別です。",
        points: [
          "再代入したい名前は let。点数のように変わるもの",
          "付け替えない名前は const。税率のように固定したいもの",
          "const は「中身が凍る」ではない。凍るのは名前の付け替え",
        ],
        talk: [
          {
            "speaker": "beginner",
            "text": "constで作ったオブジェクトは、中の欄も一切変えられないんですか？"
          },
          {
            "speaker": "engineer",
            "text": "constが禁止するのは、名前を別の値へ再代入することです。"
          },
          {
            "speaker": "beginner",
            "text": "userの年齢欄を書き換えるのも、別の値にしたことになりませんか？"
          },
          {
            "speaker": "engineer",
            "text": "userという付箋は同じオブジェクトを指したままなので、欄の更新はできます。変わる名前はlet、付け替えない名前はconstを使い、オブジェクト内部の話は参照の講義で詳しく見ます。"
          }
        ],
        diagram: "rewrite",
        code: `const tax = 0.1;
// tax = 0.08; // エラー。名前の付け替え禁止

const user = { n: 1 };
user.n = 2; // これは名前の付け替えではないので可能`,
        codeCaption: "付箋は固定、引き出しの中は別",
        codeExample: `let score = 10;
score = 20; // OK。let は付け替え可
const tax = 0.1;
console.log(tax); // 読むのはできる`,
        note: "オブジェクトの中身は参照先の変更です。後の「参照」の講義で詳しく扱います。",
      },
      {
...
console.log(age);`,
      ...
      },
      {
        id: "q4",
        slide: 3,
        prompt: "注文金額の計算で使う税率0.1を、変更しないtaxとして用意し、その値を表示してください。",
        lead: "この処理中では税率を変更しない前提です。taxへ数値0.1を関連付け、その名前から読み取った0.1が表示されれば完成です。",
        kind: "code",
        starter: "// 付け替えない名前は const\n",
        fileName: "script.js",
        steps: [
          "taxを再代入しない名前として宣言する",
          "数値0.1を関連付ける",
...
console.log(\`Hello \${name}\`);`,
      ...
      },
      {
        id: "q4",
        slide: 3,
        prompt: "入力欄から届いた商品数rawを数値へ変換し、商品を1個追加した合計を表示してください。",
        lead: "starterのrawは文字列の「3」です。そのままつながないよう数値へ変換してから1を加え、数値4が表示されれば完成です。raw自体は書き換えません。",
        kind: "code",
        starter: 'const raw = "3";\n// 数にしてから足す\n',
        fileName: "script.js",
        steps: [
...
console.log(typeof x);`,
      ...
      },
      {
        id: "q2",
        slide: 1,
        prompt: "現在の商品数countへ入力欄の追加数inputを足し、文字列のままの結果と、数値へ直した合計を順に表示してください。",
        lead: "countは数値1、inputは文字列の「2」として用意されています。1行目で型をそろえない結果12を再現し、2行目でinputを数値へ変換した正しい合計3を表示してください。",
        kind: "code",
        starter: 'const count = 1;\nconst input = "2";\n',
        fileName: "script.js",
        steps: [
...
console.log(count + Number(input));`,
        explain:
